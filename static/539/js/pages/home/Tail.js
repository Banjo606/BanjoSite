...
tail.changeData = function(dataSource, dateRange) {
	...
	tail.Chart.options.scales.yAxes[0].ticks.stepSize = (nMax >= 10 ? 0 : 1);
	// var nUpper = Math.floor(nMax * 1.05 / 5 + 0.9999) * 5;
	tail.Chart.options.scales.yAxes[0].ticks.max = (nMax <= 20 ? nMax : nMax);
	var nLower = Math.floor(nMin * 0.9);
	nLower = (nMax - nLower < 10 ? (nMax > 10 ? nMax - 10 : 0) : nLower);
	tail.Chart.options.scales.yAxes[0].ticks.suggestedMin = (nMin == 0 ? 0 : nLower);
